...
<head>
    ...
    <title>Archive de pointage</title> 
...
$statement = $pdo->prepare("TRUNCATE TABLE `archive_pointage`");// vider une table
$statement->execute();
$statement1 = $pdo->prepare("TRUNCATE TABLE `archive_rapport`");// vider une table
$statement1->execute();
...
//select le pdf_pointage from le dossier vers la base de données
 $nom_repertoire ="../pointage_pdf/archive_pointage";
   ...
   while ($fichier = readdir($pointeur))
   {
     ...
     $blob =$filePath;
        ...
        $stmt =$pdo->prepare ('INSERT INTO archive_pointage(nom,data) VALUES("'.$name_pdf.'", "'.$filePath.'")');
        $stmt->execute();
...
//select le pdf_rapport from le dossier vers la base de données
 $nom_repertoire_rapport ="../pointage_pdf/archive_rapport";
   ...
   while ($fichier = readdir($pointeur_rapport))
   {
     ...
     $blob =$filePath_rapport;
        ...
        $stmt1 =$pdo->prepare ('INSERT INTO archive_rapport(nom,data) VALUES("'.$name_pdf_rapport.'", "'.$filePath_rapport.'")');
        $stmt1->execute();
...
$query = $pdo->query("SELECT * FROM archive_pointage");
    $resultat = $query->fetchAll();

$query1 = $pdo->query("SELECT * FROM archive_rapport");
...
<div>
      <div class="page-title">
       <h2><i class="fas fa-file-archive"></i></i> Archive de pointage</h2>
        <ol class="breadcrumb" style="width:100%;height:45px; background-color: #b2b2b2;color: white">
         <li class="active" style="width:100%;"><a href="dashboard.php?id=<?php echo $id ?>" ><i class="fas fa-home"></i> Accueil</a> / <a href="archive.php?id=<?php echo $id ?>"> Archives</a> / <a href="">  Archive de pointage</a> <div id="reportrange" class="btn btn-green btn-square date-picker" style="margin-left:47%;font-size: 15px;display: inline;">
...
<div class="col-md-3">
 ...
 ?>
        ...
        <tbody>
            ...
            <tr scope="row">
                    <td colspan="3"  class="page-header" style="background-color: #880e4f;color: white;font-size: 18px;padding-top:2%;"> <?php echo $value ; ?></td>
...
<?php
      //Afficher les pdf_rapport
        ...
        $name_rappo= explode('.',$resultat1[$key]['nom'], 2);

        $name_rappo__ann=explode('-',$name_rappo[0], 2);
        
       
        if ($name_rappo__ann[1]===$value) {


    ?> 
    <tr class="toggler toggler1" >
       <td rowspan="1" style="background-color:#b2b2b2;color: black;font-size: 18px;padding-top:2%;"><?php echo  $name_rappo__ann[0]."-".$name_rappo__ann[1] ?></td>
    
                        <td  class="table-light"colspan=""  class="page-header"> <button class="btn btn-unique"><a style="color: white"href="<?php echo $resultat1[$key]['data'] ?>">Rapport</a></button>
...
<?php 
        ...
        $name_poin = explode('.',$resultat[$key]['nom'], 2);
        $name_poin=$name_poin[0];
        
        
        if ($name_poin==$name_rappo[0]) {

    ?> 

<td class="table-light" colspan=""  class="page-header">
