...
<script type="text/javascript">
  ...
  text: 'Etes vous de vouloir préparer un Pv d\'insatalation ?',
  confirmButtonText:
    '<button style="background-color:#3085d6;border:none;color:white;width:70px" onclick="document.getElementById(\'in\').click()">Oui</button>',
  ...
  footer: '<a href="ess.php?id=<?php echo $id;?>">selectioner un employé </a>'
})}
...
function error(){
 ...
 function error_format() {
  ...
  confirmButtonText:
    '<button style="background-color:#3085d6;border:none;color:white;width:70px" onclick="document.getElementById(\'add\').click()"> Ok!</button>'
})}
</script>
...
if ($resquete->execute()){ ?>
  ...
  icon: 'success',
  title: 'Vous avez supprimer l\'empoyé avec succes',
  showConfirmButton: false,
  timer: 1900
})
  }
...
if (isset($_POST['pv_insta']))
{
  $query = $pdo->query("SELECT * FROM `employé` ");
$resultat5 = $query->fetchAll();
foreach ($resultat5 as $key => $variable) {
  $i= $resultat5[$key]['idemp'];
}
$idempp=$i; ?>
  <script type="text/javascript">
 window.open('PV_recrutement.php?id=<?php echo $id; ?>&idemp=<?php echo $idempp ?>', '_blank');
    </script>
<?php }
...
if(!preg_match("#^[a-zA-Z \s]*$# ",$_POST["nom"])){
   ...
   if(!empty($_POST["NumeroTel"])){
    ...
    if(!preg_match("#^[0-9]*$# ",$_POST["Nombre_enfants"])){
      $errors['Nombre_enfants'] = "Le champs Nombre d'enfant ne tel doit contenir que des chiffres";
   }
...
Swal.fire({
  position: 'center',
  icon: 'success',
  title: 'Employés supprimés avec succès',
  showConfirmButton: false,
  timer: 1500
}) </script>
<html>
...
<script type="text/javascript"> 
 ...
 <?php }}else {?>
  ...
  showConfirmButton: false,
  footer: '<a href="ess.php ?id=<?php echo $id ?>"><i class="fa fa-thumbs-down" style="color :red;"></i> </a>'
})
  }
  fail_ajout();
 </script>
      ...
      <div class="page-title">
       <h2><i class="fa fa-users"></i>Gestion des employés </h2>
        <ol class="breadcrumb" style="width:100%;height:45px; background-color: #b2b2b2;color: white">
         <li class="active" style="width:100%;"><a href="dashboard.php?id=<?php echo $id ?>" ><i class="fas fa-home"></i> Accueil</a> / <a href="">  Gestion des employés </a> <div id="reportrange" class="btn btn-green btn-square date-picker" style="margin-left:60%;font-size: 15px;display: inline;">
...
foreach ($resultat as $key => $variable){ ?> 
                     <tr>
                      <td>
<?php if($resultat[$key]['Emploi']=="Directeur"){ ?> <p style=" background-color: transparent;"></p> <?php }else{ ?>
                      ...
                      <td>   <?php if($resultat[$key]['Emploi']=="Directeur"){ }else{ echo $resultat[$key]['Matricule']; } ?>    </td>
                      <td>   <?php echo $resultat[$key]['Emploi'] ?>   </td>
<td style="text-align: center;">
   <?php if ($resultat[$key]['Emploi']=="Directeur") {
          ...
          <a  href="ess.php?id=<?php
           ...
           <i class="fas fa-trash-alt" style="color: red;"></i></a> 
             <a  href="ess.php?id=<?php
                            ...
                            <i class="fas fa-archive" style="color: grey;"></i></a> 
                          <?php } ?>
                          </td>
...
<div class="modal fade" id="basicExampleModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true" style="background: rgba(0, 0, 0, 0.7);
  transition: opacity 500ms;
 opacity: 1;">
...
<label for="Situationfamiliale">Situation:</label>
              <select  name="situation"class="form-control browser-default custom-select" required value="<?php echo isset($_POST["situation"])  ? $_POST["situation"]: '' ?>"style=" border: none;border-bottom: 1px solid #bc5a45; height: 25px;background-color: transparent;" onblur="myFunction()" id="fname">
    ...
    <div class="form-group col-md-2" id="inputnbr" >
      ...
      <input type="text" name="Nombre_enfants"class=" <?php if(show_error('Nombre_enfants')){ echo 'form-control is-invalid'; }else{ echo 'form-control'; }?>"  id="inputZip" value="<?php echo isset($_POST["Nombre_enfants"])  ? $_POST["Nombre_enfants"]: '' ?>"style=" border: none;border-bottom: 1px solid #bc5a45; background-color: transparent;height: 25px"><div class="invalid-feedback">
...
$f = $f->fetch();*/
    ...
    <!-- Default input -->
      ...
      <select name="emploi" class="form-control browser-default custom-select" required value="<?php echo isset($_POST["emploi"])  ? $_POST["emploi"]: '' ?>"style=" border: none;border-bottom: 1px solid #bc5a45; background-color: transparent;height: 25px;padding-top: 0%" id="fEmploi" 
        onblur="myFunction2()">
    ...
    <div class="form-group col-md-2">
      ...
      <div class="invalid-feedback">
          <?php echo show_error('diplome');?>
               ...
               <div class="form-group col-md-1" id="grp">
            <label for="role">Groupe:</label>
          <select name="Groupe"style=" border: none;border-bottom: 1px solid #bc5a45; background-color: transparent;height: 25px;padding-top: 0%" class="form-control browser-default custom-select" value="groupe">
...
$fonction = $fonction->fetchAll();
    ?>
            <select name="fonct" class="form-control browser-default custom-select" value="<?php echo isset($_POST["fonct"])  ? $_POST["fonct"]: '' ?>"style=" border: none;border-bottom: 1px solid #bc5a45; background-color: transparent;height: 25px;padding-top: 0%;margin-left: 2%">
...
<label for="Situationfamiliale">Situation:</label>
    ...
    <div class="form-group col-md-2">
      ...
      <input type="text" name="NumeroTel" class=" <?php if(show_error('NumeroTel')){ echo 'form-control is-invalid'; }else{ echo 'form-control'; }?>" id="inputCity" required value="<?php echo isset($resultatt[$key]['Numero_Tel'])  ? $resultatt[$key]['Numero_Tel']:  $_POST["NumeroTel"] ?>"  style=" border: none;border-bottom: 1px solid #bc5a45; background-color: transparent;height: 25px"><div class="invalid-feedback">
          <?php echo show_error('NumeroTele');?>
        </div> 
      </div>
...
$stat = $statuu->fetchAll();
 ...
 ?> </option>
    ...
    <!-- Default input -->
          ...
          <div class="form-group col-md-1">
                    <?php 
                    $f = $pdo->query("SELECT * FROM `employé` WHERE idemp='$idemp'");
$ff = $f->fetchAll();
foreach ($ff as $key => $value) { 
$ech=$resultatt[$key]['Echelon']; }
?>
            <label for="Echlon">Echlon:</label>
            <select type="gender" name="Echlon" class="form-control browser-default custom-select"  required value="<?php echo isset($ech)  ? $ech:  $_POST["Echlon"]; ?>" style=" border: none;border-bottom: 1px solid #bc5a45; background-color: transparent;height: 25px;padding-top: 0%">
                <?php for ($i=1; $i <14 ; $i++) {
              ?>
                <option value="<?php echo $i ?>" <?php if($ech==$i){ ?> selected <?php } ?>><?php echo $i; ?></option>
              <?php } ?>
            </select>
          </div>
             <div class="form-group col-md-2">
...
$ff = $f->fetchAll();
foreach ($ff as $key => $value) { 
$s=$resultatt[$key]['service']; }
?>
      <label for="">Service</label>
      <input type="text" name="service"class=" form-control " required value="<?php echo isset($s)  ? $s:  $_POST["service"] ?>" style=" border: none;border-bottom: 1px solid #bc5a45; background-color: transparent;height: 25px"> 
        </div>
<?php 
      $f = $pdo->query("SELECT * FROM `employé` WHERE idemp='$idemp'");
$ff = $f->fetchAll();
foreach ($ff as $key => $value) { 
$e=$resultatt[$key]['email']; }
?>
                        <div class="form-group col-md-2">
             <label for="">email</label>
      <input type="email"
      name="email" required value="<?php echo isset($e)  ? $e:  $_POST["email"] ?>" class="form-control"
       style=" border: none;border-bottom: 1px solid #bc5a45; background-color: transparent;height: 25px;">
        <div class="invalid-feedback">
...
<form method="POST">
    <button type="submit" class="hidden" name="pv_insta" ></button>
  </form>
</div>
</body>
<style type="text/css">
...
if (x.value === "Marié" || x.value == "Divorcé") {
      document.getElementById("inputnbr").style.display = "block";

}}
function myFunction2() {
  var x = document.getElementById("fEmploi");
if (x.value == "Enseignant") {
